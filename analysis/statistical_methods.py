...
def collect_metrics_for_pair(stock_1, stock_2) -> pd.DataFrame:
    ...
    stock_data_df["return"] = (
        stock_data_df["Adj Close"] - stock_data_df["Open"]
    ) / stock_data_df["Open"]
    stock_data_df["forward_return"] = stock_data_df.groupby(level=1)[
        "return"
    ].transform(lambda x: x.shift(-1))
    stock_data_df = (
        stock_data_df[["Adj Close", "forward_return"]]
        .unstack()
    ...
    stock_data_df.columns = [
        stock_1,
        stock_2,
        f"{stock_1}_forward_return",
        f"{stock_2}_forward_return",
    ]
    ...
    stock_data_df["roll_corr"] = (
        stock_data_df[stock_1].rolling(180).corr(stock_data_df[stock_2])
    )

    def smooth_zscore(spread):
        return (spread.rolling(1).mean() - spread.rolling(50).mean()) / spread.rolling(
            50
        ).std()
    ...
    try:
        ...
        if len(highest_corr_pairs) != 0:
            for n in range(len(highest_corr_pairs)):
                result = adf_test(
                    highest_corr_pairs["Stock_1"][n], highest_corr_pairs["Stock_2"][n]
                )
