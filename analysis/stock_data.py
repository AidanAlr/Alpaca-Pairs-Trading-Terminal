...
class StockData:
    ...
    def __init__(self, asset_list, bypass_adf_test):
        ...
        self.co_integrated_pairs_df = self.find_cointegrated_pairs(
            self.price_history_df, p_value_thresh=0.05
        )
    ...
    def find_highest_corr_pairs(self, df):
        ...
        cmu = cmu.reset_index()
        cmu = cmu.rename(
            columns={"level_0": "Stock_1", "level_1": "Stock_2", 0: "Correlation"}
        )
        highest_corr_pairs = cmu[cmu["Correlation"] >= 0.80]
        highest_corr_pairs["lookup"] = (
            highest_corr_pairs["Stock_1"] + " - " + highest_corr_pairs["Stock_2"]
        )
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if p_value <= p_value_thresh:
                    cointegrated_pairs_dict[f"{df.columns[i]} - {df.columns[j]}"] = (
                        p_value
                    )

        cointegrated_pairs_df = pd.DataFrame.from_dict(
            cointegrated_pairs_dict, orient="index"
    ...
    def combine_cointegration_correlation(self) -> pd.DataFrame:
        ...
        coint_corr_data = self.highest_corr_pairs_df.merge(
            self.co_integrated_pairs_df,
            left_on="lookup",
