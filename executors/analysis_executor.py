...
def run_analysis() -> None:
    ...
    while True:
        try:
            ...
            blue_bold_print(
                "Please enter the absolute path to a csv file containing a list of ticker symbols, leaving this blank will use the default symbols.csv or enter b to go "
                "back:"
            )
            path = input()
            if path == "b":
...
def execute_pairs_strategy(pair: List):
    ...
    if alpaca.in_position:
        red_bold_print(
            "You are currently in a position, please exit the position before executing a new strategy."
        )
    else:
        if not pair:
            try:
                ...
                logging.info(
                    f"Tickers to execute strategy are: {pair[0]} and {pair[1]}"
                )
                strategy_info = collect_metrics_for_pair(pair[0], pair[1])
                hedge_ratio = strategy_info["hedge_ratio"].iloc[0]
                ...
                red_bold_print(
                    "Enter your take profit and stop loss in the format 0.1, 0.05"
                )
                tp, sl = input().split(",")
                tp = float(tp.strip())
                sl = float(sl.strip())
                ...
                if confirmed:
                    ...
                    while True:
                        ...
                        if alpaca.check_and_stop_loss(
                            sl
                        ) or alpaca.check_and_take_profit(tp):
                        ...
                        if alpaca.in_position:
                            logging.info(
                                "You are currently in a position so will not execute a new trade."
                            )
...
def backtest_strategy(pair: List or Tuple) -> None:
    ...
    if not pair:
        blue_bold_print(
            "Please enter the stock ticker you would like to backtest in the format stock_1, stock_2:"
        )
        pair_input = input()
